...
const transformMethods = (root, path) => {
    ...
    for (var propertyName in root) {
        ...
        var _parameters = transformParameters(value.parameters);
        operations.push({
            authorizations: {
                oauth2: ["", ""]
            },
            ...
            parameterString: paramHelper.GetFunctionParameter(_parameters),
            requests: transformRequestParamerters(_parameters)
        });
    }
    return operations;
};

const transformRequestParamerters = parameters =>{
    var params = [];
    for (var propertyName in parameters){
        var value = parameters[propertyName];
        params.push(
            {
                requestFunction: paramHelper.GetRequestParameter(value)
            }
        )
    }
    return params;
